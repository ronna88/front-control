...
const PFTable = ({rows, setRows, loading, setLoading, edit, setEdit,
    erase, setErase}) => {
    ...
    function handleEditClick(pf){
        setSelectedPF(pf)
        setEdit(true)
        setErase(false)
        handleOpen()
    }
    function handleDeleteClick(pf){
        setSelectedPF(pf)
        setErase(true)
        setEdit(false)
        handleOpen()
                ...
                (loading ? (
                  ...
                  (rows?.content).map((row) => (
                      <TableRow key={row.clienteId}>
                          ...
                          <TableCell>{row.clienteCPF}</TableCell>
    ...
    <Modal
        ...
        <Box sx={style}>
            <Typography sx={{marginBottom: '2rem'}} id="modal-modal-title" variant="h3" component="h2">
                {edit ? 'Editar' : 'Apagar'} cliente PF
            </Typography>
            <PFForm handleOpen={handleOpen} handleClose={handleClose}
                setRows={setRows} setLoading={setLoading} edit={edit} setEdit={setEdit}
                erase={erase} selectedPF={selectedPF}
