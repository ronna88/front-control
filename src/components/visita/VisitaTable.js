...
const VisitaTable = ({
  ...
  const [form, setForm] = useState({
    ...
    visitaDescricao: '',
    visitaRemoto: false,
    visitaValorProdutos: 0.0,
    visitaTotalAbono: 0.0,
    funcionarios: [],
    cliente: '',
    local: '',
    visitaTotalHoras: 0.0,
  });
  ...
  */
  function getFuncionarios(visita) {
    ...
    for (let i = 0; i < visita.funcionarios.length; i++) {
      const result = listaFuncionarios.filter(
        (func) => func.funcionarioId === visita.funcionarios[i].funcionarioId,
      );
    ...
    <>
      <Table>
        ...
        <TableBody>
          {loading ? (
            <TableRow>
              <TableCell colSpan="5">Sem dados...</TableCell>
            </TableRow>
          ) : (
            rows?.content?.map((row) => (
              <TableRow key={row.visitaId}>
                ...
                <TableCell
                  style={{
                    textDecoration: row.visitaTotalAbono > 0 ? 'line-through' : 'none',
                    color: row.visitaTotalAbono > 0 ? 'red' : 'black',
                  }}
                >{`${row.visitaDescricao}`}</TableCell>
